...
const AppAdminPage: React.FC = () => {
    ...
    const setNew = (record: API.App) => {
        ...
        setAppFeatured({id: record.id, priority: nextPriority})
            .then(() => {
     ...
     * @returns 表格数据
     */
    const fetchApps = async (params, sort, filter) => {
    ...
    const columns: ProColumns<API.App>[] = [
        ...
        },
        {
            ...
            ellipsis: true,
            render: (text) => (
                <span title={text as string}
                      style={{maxWidth: `${COLUMN_WIDTHS.initPrompt}px`, display: 'inline-block'}}>
          {text}
        </span>
            ),
        },
