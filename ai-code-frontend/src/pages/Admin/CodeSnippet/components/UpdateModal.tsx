import { updateCodeSnippet } from '@/services/backend/codeSnippetController';
import { ProTable } from '@ant-design/pro-components';
import type { ProColumns } from '@ant-design/pro-table';
import { message, Modal } from 'antd';
import React from 'react';
import CodeEditor from '@/components/CodeEditor';

interface Props {
  oldData?: API.CodeSnippet;
  visible: boolean;
  columns: ProColumns<API.CodeSnippet>[];
  onSubmit: (values: API.CodeSnippet) => void;
  onCancel: () => void;
}

const handleUpdate = async (fields: API.CodeSnippet) => {
  const hide = message.loading('正在更新');
  try {
    await updateCodeSnippet(fields);
    hide();
    message.success('更新成功');
    return true;
  } catch (error: any) {
    hide();
    message.error('更新失败，' + error.message);
    return false;
  }
};

const UpdateModal: React.FC<Props> = (props) => {
  const { oldData, visible, columns, onSubmit, onCancel } = props;

  if (!oldData) {
    return <></>;
  }

  const customColumns = columns.map(col => {
    if (col.dataIndex === 'snippetCode') {
      return {
        ...col,
        renderFormItem: (_, { type, defaultRender, ...rest }, form) => {
          return (
            <CodeEditor
              value={form.getFieldValue('snippetCode')}
              onChange={(value) => form.setFieldValue('snippetCode', value)}
              placeholder="请输入代码片段..."
              rows={15}
            />
          );
        },
      };
    }
    return col;
  });

  return (
    <Modal
      destroyOnClose
      title={'更新代码模板'}
      open={visible}
      footer={null}
      onCancel={() => {
        onCancel?.();
      }}
      width={800}
    >
      <ProTable
        type="form"
        columns={customColumns}
        form={{
          initialValues: oldData,
        }}
        onSubmit={(values: API.CodeSnippet) => {
          handleUpdate({
            ...values,
            id: oldData.id,
          }).then(success => {
            if (success) {
              onSubmit?.(values);
            }
          });
        }}
      />
    </Modal>
  );
};
export default UpdateModal;
